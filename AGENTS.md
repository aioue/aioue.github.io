...
- **Heading permalinks:** kramdown `auto_ids` plus `assets/js/heading-anchors.js` add a `#` link on h2-h6. Slugs are the heading text, lowercased, spaces to hyphens (e.g. `#key-steps-explained`). Do not hand-write HTML ids on headings unless you need to override a slug.
