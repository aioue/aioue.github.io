...
- **Heading permalinks:** kramdown `auto_ids` plus `assets/js/heading-anchors.js`. A `#` sits in the left gutter (GitHub README / [Ben Balter](https://ben.balter.com/2014/03/13/pages-anchor-links/) pattern) and is visible only on heading hover. Slugs are the heading text, lowercased, spaces to hyphens (e.g. `#key-steps-explained`).
