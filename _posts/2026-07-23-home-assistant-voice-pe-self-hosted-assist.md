...
I wanted a local voice puck without reviving a lapsed Nabu Casa subscription. The [Home Assistant Voice Preview Edition](https://www.home-assistant.io/voice-pe/) is ESP32-S3 hardware with Wyoming STT/TTS into Assist on my HA VM (101 on Proxmox, Ryzen 5650G). After an evening of USB and language-code friction it works. I skipped Alexa-style "study off" room control; announcements and Q&A are enough.
...
Alexa maps "study off" to a room light group. I had 24 study-related entities exposed to Assist: 14 Hue scenes (`Study Bright`, `Study Relax`, …), four lights, two Cast players named `study`, plus sensors. Assist answered:

> There are multiple devices called study

You can fix that with tighter exposure (room groups only) or a custom `{area} off` sentence. I did not want to maintain per-room voice aliases on this puck, so I use it for prompts and Q&A instead (morning study motion -> weather, chore check-in; maybe `ask_question` later). OpenAI Conversation (`gpt-4o-mini`, `prefer_local_intents`) when open-ended chat is worth the API cost.

## Redo list

If I set this up again:

- Flash from a direct Mac USB-C port; skip hubs and AV adapters
- Set the external URL (no `:8123`) before "Add to Home Assistant"
- Set `tts_language: en_GB` before the first Try
- Do not expose every Hue scene to Assist if you care about room names

Planned automation: `binary_sensor.top_study_motion_osram_occupancy` -> `assist_satellite.announce` on `assist_satellite.home_assistant_voice_09414f_assist_satellite` with weather + chore state. Later: `ask_question` for yes/no, then OpenAI for follow-up chat.
